...
use std::{
    collections::{HashMap, HashSet}, sync::mpsc::Receiver
};

pub(crate) type ResourceLoadingFunction = std::sync::Arc<dyn 
    Fn() -> Result<ResourceHandle, ErrorType>
+ Send + Sync>;
...
pub(crate) trait ResourceLoadingParameters<R: Resource>: std::hash::Hash + std::any::Any + Send + Sync + std::fmt::Debug + Clone + 'static {
...
impl LoadingResource {
    pub(crate) fn new(resource_id: &UserResourceId, loading_function: &ResourceLoadingFunction) -> Result<Self, ErrorType> {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if let Err(err) = sender.send(data) {
                    log_error!("Failed to send data between threads when loading a resource: {:?}", err);
                    panic!();
                };
        ...
        {
            log_error!("Failed to build a thread when loading a resource: {:?}", err);
            return Err(ErrorType::Unknown);
        }
...
impl ResourceIdGenerator {
    ...
    pub(crate) fn insert(&mut self, id: &UserResourceId, real_id: &ResourceId) -> Result<(), ErrorType> {
...
impl ResourceManager {
    ...
    pub(crate) fn add(&mut self, user_id: &UserResourceId, resource_type_id: &std::any::TypeId, loading_function: ResourceLoadingFunction) 
        -> Result<(), ErrorType>
    {
        // Generate real id
        ...
        match GLOBAL_RESOURCE_ID_GENERATOR.write() {
            Ok(mut generator) => {
                if let Err(err) = generator.insert(user_id, &real_id) {
                    log_error!("Failed to add a user id to real id entry in the resource id generator: {:?}", err);
                    return Err(ErrorType::Unknown);
                }
                if let Err(err) = generator.insert_inv(&real_id, user_id) {
                    log_error!("Failed to add a real id to user id entry in the resource id generator: {:?}", err);
                    return Err(ErrorType::Unknown);
                }
            },
            Err(err) => {
        ...
        if !self.real_resources.contains_key(resource_type_id) {
            let _ = self.real_resources.insert(*resource_type_id, HashSet::new());
        }
        ...
        if self.loading_functions.insert(real_id, loading_function).is_some() {
            log_error!("Failed to add a loading function when adding a new resource in the manager");
            return Err(ErrorType::Duplicate);
        }
    ...
    pub(crate) fn get_all_ids(&self, resource_type_id: &std::any::TypeId) -> Result<&HashSet<ResourceId>, ErrorType> {
        if !self.real_resources.contains_key(&resource_type_id) {
            log_error!("Failed to get all resource ids of the `{:?}' resource type from the resource manager", 
                resource_type_id
            );
    ...
    pub(crate) fn try_get(&mut self, id: &ResourceId, resource_type_id: &std::any::TypeId) -> Result<Option<ResourceHandle>, ErrorType> {
        ...
        match self.resources.get_mut_entry(id){
            Ok(entry) => {
                match entry {
                    super::generational::Entry::Free {..} => {
                        log_error!("Invalid real resource id found when trying to access resource in the resource manager");
                        Err(ErrorType::DoesNotExist)
                    },
                    super::generational::Entry::Occupied { value } => match value {
                        ...
                        None => {
                            let loading_function = match self.loading_functions.get(id) {
                                None => {
                                    log_error!("Failed to find a loading function when trying to access resource in the resource manager");
                                    return Err(ErrorType::DoesNotExist);
                                },
                                Some(fct) => fct,
                            };
                        ...
                        Some(RealResource::Loading(loading_resource)) => {
                            match loading_resource.receiver.try_recv() {
                                Ok(data) => {
                                    // TODO: add check
                                    let _ = self.loading_resources.remove(&(*resource_type_id, *id));
                                    let handler = ResourceHandle::clone(&data);
                                    *value = Some(RealResource::Loaded(LoadedResource { handler: data }));
                                    Ok(Some(handler))
                                }
                                ...
                                Err(err) => {
                                    log_error!("Failed to load a resource in the resource manager: {:?}", err);
                                    Err(ErrorType::Unknown)
                                }
                            }
                        },
            ...
            Err(err) => {
                log_error!("Failed to get an entry when trying to access resource in the resource manager: {:?}", err);
                Err(ErrorType::Unknown)
            },
        }
    }

    pub(crate) fn get(&mut self, id: &ResourceId, resource_type_id: &std::any::TypeId) -> Result<ResourceHandle, ErrorType> {
        loop {
            match self.try_get(id, resource_type_id) {
                Err(err) => {
                    log_error!("Failed to get the `{:?}' resource from the resource manager: {:?}", id, err);
                    return Err(ErrorType::Unknown);
                },
