    ...
    fn new(device: &ash::Device, queue_families: &VkQueueFamilies) -> Result<Self, ErrorType> {
        let graphics: Vec<ash::vk::Queue> = (0..queue_families.graphics.count)
            .map(|queue_index| unsafe {
                device.get_device_queue(queue_families.graphics.index as u32, queue_index as u32)
            })
            .collect();
        ...
        let compute: Vec<ash::vk::Queue> = (0..queue_families.compute.count)
            .map(|queue_index| unsafe {
                device.get_device_queue(queue_families.compute.index as u32, queue_index as u32)
            })
            .collect();
        if compute.is_empty() {
        ...
        let transfer: Vec<ash::vk::Queue> = (0..queue_families.transfer.count)
            .map(|queue_index| unsafe {
                device.get_device_queue(queue_families.transfer.index as u32, queue_index as u32)
            })
            .collect();
...
impl VkQueueFamilies {
    ...
    fn new(queue_family_properties: &[ash::vk::QueueFamilyProperties]) -> Result<Self, ErrorType> {
        let graphics = match VkQueueFamily::new_graphics(queue_family_properties) {
            ...
            Err(err) => {
                log_error!(
                    "Failed to create a graphics queue family when initializing the Vulkan queues: {:?}",
                    err
                );
        ...
        let compute = match VkQueueFamily::new_compute(queue_family_properties) {
            ...
            Err(err) => {
                log_error!(
                    "Failed to create a compute queue family when initializing the Vulkan queues: {:?}",
                    err
                );
                return Err(ErrorType::Unknown);
            }
...
pub(crate) fn init_device(
    ...
    let queue_families = match VkQueueFamilies::new(&queue_family_properties) {
        ...
        Err(err) => {
            log_error!(
                "Failed to create the queue families when initializing Vulkan device: {:?}",
                err
            );
            return Err(ErrorType::Unknown);
        }
    ...
    let graphics_queue_priorities =
        vec![queue_families.graphics.priority; queue_families.graphics.count];
    let compute_queue_priorities =
        vec![queue_families.compute.priority; queue_families.compute.count];
    let transfer_queue_priorities =
        vec![queue_families.transfer.priority; queue_families.transfer.count];
    let mut device_queue_info = vec![
    ...
    if queue_families.compute.index != queue_families.graphics.index {
        device_queue_info.push(
            ash::vk::DeviceQueueCreateInfo::default()
                .queue_family_index(queue_families.compute.index as u32)
                .queue_priorities(&compute_queue_priorities),
        );
    }
    if queue_families.transfer.index != queue_families.graphics.index
        && queue_families.transfer.index != queue_families.compute.index
    {
        device_queue_info.push(
            ash::vk::DeviceQueueCreateInfo::default()
    ...
    let required_extensions = match VkExtension::to_vknames(
        ...
        Err(err) => {
            log_error!(
                "Failed to get the required device extensions names when initializing Vulkan device: {:?}",
                err
            );
            return Err(ErrorType::Unknown);
        }
    ...
    let mut required_features_extended_dynamic_state = VkFeaturesExt::as_extended_dynamic_state(
        &config
            .renderer_config
            .vulkan_parameters
            .required_device_features_ext,
    );
    let mut required_features_extended_dynamic_state2 = VkFeaturesExt::as_extended_dynamic_state2(
        &config
            .renderer_config
            .vulkan_parameters
            .required_device_features_ext,
    );
    let mut required_features_extended_dynamic_state3 = VkFeaturesExt::as_extended_dynamic_state3(
        &config
            .renderer_config
            .vulkan_parameters
            .required_device_features_ext,
    ...
    let mut required_features = ash::vk::PhysicalDeviceFeatures2::default()
        ...
        // TODO: add other ext features
        .push_next(&mut required_features_extended_dynamic_state)
        .push_next(&mut required_features_extended_dynamic_state2)
        .push_next(&mut required_features_extended_dynamic_state3);
    ...
    let queues = match VkQueues::new(&device, &queue_families) {
        ...
        Err(err) => {
            log_error!(
                "Failed to get the queues when initializing the Vulkan device: {:?}",
                err
            );
            return Err(ErrorType::Unknown);
        }
